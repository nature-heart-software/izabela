...
class GameOverlay {
    ...
    this.Overlay!.setEventCallback((event: string, payload: any) => {
      ...
      if (event === 'game.input.intercept' && payload.intercepting) {
        ...
        if (focusWin) {
          ...
          const { top, left, right, bottom } = Window.getByPid(payload.pid).getDimensions()
          const width = right-left
          const height = bottom-top
          // focusWin.setSize(width, height)

          mouse.getPosition()
            .then(async (initialPosition) => {
              await mouse.setPosition(new Point(left+width / 2, top+height / 2))
  ...
  public injectByPID(pid: number) {
    console.log(pid, this.Overlay.getTopWindows())
  ...
  public start() {
    OverlayImport.then(() => {
      ...
      child.on('message', ({ type, payload }: any) => {
        if (type === 'game-detection') {
          setTimeout(() => {
            this.injectByPID(payload.pid)
          }, 5000)
        }
      })
