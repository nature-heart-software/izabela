// credit to https://github.com/jon-hall/hott
...
export default (() => {
  ...
  const winapi = new ffi.Library('User32', {
    ...
    UnregisterHotKey: ['bool', ['int32', 'int32']],
  })
  const getKey = (keys: Key[]) => {
    ...
    const key = findLast(
      ...
      (k) =>
        !!keyCodes[
          (keyCodeMap[k.code] ||
            (keyMap[k.code] as IWindowsKeyMapping | undefined)?.vkey ||
            '') as keyof typeof keyCodes
        ],
    )
    return (
      key &&
      keyCodes[
        (keyCodeMap[key.code] ||
          (keyMap[key.code] as IWindowsKeyMapping | undefined)?.vkey ||
          '') as keyof typeof keyCodes
      ]
    )
  }
  const getModifiers = (keys: Key[]) => {
    const keyCodeMap: any = {
    ...
    const index = findLastIndex(
      ...
      (k) =>
        !!keyCodes[
          ((keyMap[k.code] as IWindowsKeyMapping | undefined)?.vkey || '') as keyof typeof keyCodes
        ],
    )
    return keys
      .slice(0, index)
      .map((k) => modifierCodes[(keyCodeMap[k.code] || '') as keyof typeof modifierCodes])
      .filter(isNumber)
  }
  ...
  return {
    register(
      ...
      if (
        winapi.RegisterHotKey(
          ...
          (modifiers && modifiers.reduce((acc, m) => acc | (m || 0), 0)) || 0,
          key,
        )
      ) {
        commands[accelerator] = id
      } else {
        console.log("[node-shortcuts] Couldn't register shortcut", accelerator)
      }
    },
    unregister(keysOrAccelerator: Key[] | string) {
      const accelerator =
        typeof keysOrAccelerator === 'string'
          ? keysOrAccelerator
    ...
    unregisterAll() {
      forEach(commands, (id, accelerator) => {
        if (winapi.UnregisterHotKey(0, id)) {
          delete commands[accelerator]
        }
      })
