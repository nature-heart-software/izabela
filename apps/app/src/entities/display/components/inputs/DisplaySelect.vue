<template>
  ...
  >
    <template v-for="display in displays" :key="display.id">
      <NvOption :value="display.id" :label="`${primaryDisplay && display.id === primaryDisplay.id && '(Primary) ' || ''}${display.id}`">
        <template v-if="primaryDisplay && display.id === primaryDisplay.id">
          (Primary)
        </template>
        {{ display.id }}
      </NvOption>
...
const isFetching = computed(() => [isAllDisplayFetching.value, isPrimaryDisplayFetching.value].some(Boolean))
