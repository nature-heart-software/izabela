      ...
      <NvStack spacing="4">
        ...
        <div class="pl-8">
          <NvCard>
            <NvStack spacing="5">
              <div>
                <NvButton size="sm" @click="store.dispatch('dictionary/addDefinition')">Add definition</NvButton>
              </div>
              ...
              <template v-for="(definition, i) in store.getters['dictionary/definitions']" :key="i">
                <NvGroup class="w-full" grow>
                  <NvInput
                    ...
                    @update:modelValue="
                      (value) => store.dispatch('dictionary/setProperty', [`persisted.definitions[${i}][0]`, value])
                    "
                  />
                  <NvDivider direction="vertical" class="!grow-0 h-5" />
                  <NvInput
                    ...
                    @update:modelValue="
                      (value) => store.dispatch('dictionary/setProperty', [`persisted.definitions[${i}][1]`, value])
                    "
                  />
                  <NvButton
                    class="!grow-0"
