...
class $fd49ddfa76b1bd76$var$GenerateExportsWebpackPlugin {
    ...
    generateIndex(directory1, options2) {
        ...
        ($parcel$interopDefault($lWZZX$globby))(include, {
            ...
            const indexTemplate = this.getTemplate(directory1, options2, filteredFiles);
    ...
    getTemplate(directory2, options3, files) {
        const { btfs: btfs  } = this;
        const { omitExtension: omitExtension , omitSemi: omitSemi , singleQuote: singleQuote  } = options3;
 ...
 */\n${files.sort((a, b)=>a.localeCompare(b)
            ...
            const exportRelativePath = btfs(btfs(filePath).replace(btfs(directory2), '').replace(omitExtension ? /\.[^/.]+$/ : '', ''));
            return `export { default as ${name} } from ${quoteType}${exportRelativePath.startsWith('/') ? `.${exportRelativePath}` : `./${exportRelativePath}`}${quoteType}${omitSemi ? '' : ';'}`;
