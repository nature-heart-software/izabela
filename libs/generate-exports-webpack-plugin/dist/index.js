...
class $fd49ddfa76b1bd76$var$GenerateExportsWebpackPlugin {
  ...
  apply() {
    this.options.directories.forEach((directoryConfig) => {
      ...
      let options = directoryConfigIsArray
        ...
        : this.options
      if (!$parcel$interopDefault($lWZZX$path).isAbsolute(directory))
        directory = $parcel$interopDefault($lWZZX$path).resolve(directory)
      const watcher = $parcel$interopDefault($lWZZX$chokidar).watch(
        options.include,
        {
          ignored: /^\./,
          cwd: directory,
  ...
  handleFileChange(filePath1, directory, options) {
    if (
      filePath1 !==
      $parcel$interopDefault($lWZZX$path).join(directory, options.filename)
    )
  ...
  async fileExists(path1) {
    return !!(await $parcel$interopDefault($lWZZX$fspromises)
      .stat(path1)
      .catch((e) => false))
  }
  generateIndex(directory1, options2) {
    if (!$parcel$interopDefault($lWZZX$path).isAbsolute(directory1))
      directory1 = $parcel$interopDefault($lWZZX$path).resolve(directory1)
    const { btfs: btfs } = this
    const { filename: filename, include: include, exclude: exclude } = options2
    const indexPath = btfs(
      $parcel$interopDefault($lWZZX$path).join(directory1, filename),
    )
    $parcel$interopDefault($lWZZX$globby)(include, {
      ...
      .then((files) => {
        ...
        return this.fileExists(indexPath)
          .then(async (exists) => {
            if (exists) {
              const fileContent = await $parcel$interopDefault(
                $lWZZX$fspromises,
              ).readFile(indexPath, 'utf8')
              if (fileContent === indexTemplate)
                return Promise.reject('Contents are identical.')
            }
            return $parcel$interopDefault($lWZZX$fspromises).mkdir(
              $parcel$interopDefault($lWZZX$path).parse(indexPath).dir,
              {
                recursive: true,
              },
            )
          ...
          .then(() =>
            $parcel$interopDefault($lWZZX$fspromises).writeFile(
              indexPath,
              indexTemplate,
            ),
          )
...
**/\n${files
      .map((filePath) => {
        const { name: name } =
          $parcel$interopDefault($lWZZX$path).parse(filePath)
        const exportRelativePath = btfs(
          btfs(filePath)
            .replace(btfs(directory2), '')
