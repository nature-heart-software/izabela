...
ipcMain.on(
  ...
  (event, targetWebContentsId, channel, ...args) => {
    const targetWebContents = webContents.fromId(targetWebContentsId)
...
function broadcastWindowId(newWindowName, newWebContentsId) {
  registeredWindows.forEach((webContentsId) => {
    webContents
      ...
      .send(INTERNAL_CHANNELS.announceWindowId, newWindowName, newWebContentsId)
  })
}
...
function sendTo(windowName, channel, ...args) {
  ...
  webContents
    .fromId(registeredWindows.get(windowName))
    .send(prefixed(MAIN_PROCESS_ID, channel), ...args)
}
...
function handle(windowName, channel, callback) {
  if (typeof callback === 'function') {
    ipcMain.handle(prefixed(windowName, channel), (event, ...args) =>
      callback(...args),
    )
  }
}
...
function handleOnce(windowName, channel, callback) {
  if (typeof callback === 'function') {
    ipcMain.handleOnce(
      prefixed(windowName, channel, (event, ...args) => callback(...args)),
    )
  }
}
